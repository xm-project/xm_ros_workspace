...
#encoding:utf8 
import roslib
roslib.load_manifest('xm_velocity_smoother')
import rospy
...
def main():
    ...
    rospy.init_node("test_velocity_smoother_input")#namespace
    cmd_vel_publisher = rospy.Publisher("~cmd_vel", Twist)
    odom_publisher = rospy.Publisher("~odom", Odometry)
    param = {}#dict
    param['velocity_maximum'] = rospy.get_param("~velocity_maximum", 0.3)
    ...
    publish = True
    #period = 0.01
    timer = rospy.Rate(100) # 100hz
    rospy.loginfo("Test Input : STATE_RAMP_UP")#状态机
    # while(1)
    while not rospy.is_shutdown():
        ...
        if state == STATE_RAMP_UP:
            ...
            cmd_vel.linear.x = cmd_vel.linear.x - param['ramp_decrement']#一直减速
